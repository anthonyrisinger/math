...
from core.gamma import gamma_safe, gammaln_safe, factorial_extension, beta_function
...
class TestGammaRecurrenceRelations:
    ...
    def test_gamma_recurrence_positive(self, z):
        ...
        assert relative_error < 1e-12, f"Γ({z}+1) ≠ {z}·Γ({z}): {gamma_z_plus_1} ≠ {expected}"

    @given(st.floats(min_value=-49.9, max_value=-0.1, allow_nan=False, allow_infinity=False))
    ...
    def test_gamma_recurrence_negative(self, z):
        ...
        assert relative_error < 1e-10, f"Γ({z}+1) ≠ {z}·Γ({z}): {gamma_z_plus_1} ≠ {expected}"

    @given(arrays(dtype=np.float64, shape=st.integers(1, 10), 
    ...
    def test_gamma_recurrence_vectorized(self, z_array):
        ...
        relative_errors = np.abs(gamma_z_plus_1_finite - expected) / np.maximum(np.abs(expected), NUMERICAL_EPSILON)
        
        assert np.all(relative_errors < 1e-12), f"Vectorized recurrence failed: max error = {np.max(relative_errors)}"
...
class TestGammaSymmetryProperties:
    ...
    def test_reflection_formula(self, z):
        ...
        assert relative_error < 1e-10, f"Reflection formula failed for z={z}: {product} ≠ {expected}"
    ...
    def test_integer_factorial_property(self, n):
        ...
        assert abs(gamma_result - expected) < NUMERICAL_EPSILON, f"Γ({n+1}) ≠ {n}!: {gamma_result} ≠ {expected}"
        assert abs(factorial_result - expected) < NUMERICAL_EPSILON, f"factorial_extension({n}) ≠ {n}!: {factorial_result} ≠ {expected}"
...
class TestGammaLogSpace:
    ...
    def test_log_gamma_consistency(self, z):
        ...
        if np.isfinite(gamma_z) and gamma_z > 0:
            ...
            assert error < 1e-12, f"log(Γ({z})) inconsistent: {log_gamma_z} ≠ {expected_log}"
    ...
    def test_log_gamma_recurrence(self, z):
        ...
        assert error < 1e-12, f"Log-gamma recurrence failed for z={z}: {log_gamma_z_plus_1} ≠ {expected}"
...
class TestBetaFunctionProperties:
    ...
    @given(st.floats(min_value=0.1, max_value=5.0, allow_nan=False, allow_infinity=False),
    ...
    def test_beta_gamma_relation(self, a, b):
        ...
        assume(np.isfinite(gamma_a) and np.isfinite(gamma_b) and np.isfinite(gamma_a_plus_b))
        ...
        assert relative_error < 1e-10, f"Beta-gamma relation failed: B({a},{b}) = {beta_ab} ≠ {expected}"
