...
from core_measures import PHI, PI, DimensionalMeasures, setup_3d_axis
        ...
        for target in range(1, self.max_dim):
            ...
            for source in range(target):
                if abs(self.phase_density[source]) > 0.01:
                    ...
                    transfer = min(
                        transfer, abs(self.phase_density[source]) * 0.5
                    )  # Prevent overdrain
        ...
        for d in range(1, self.max_dim):
            if d not in self.emerged:
                ...
                if current_phase >= capacity * 0.8:  # 80% threshold
                    self.emerged.add(d)
                    print(
                        f"✨ Dimension {d} emerged! (phase: {current_phase:.3f}, capacity: {capacity:.3f})"
                    )
...
class PhaseDynamicsVisualizer:
    ...
    def create_figure(self):
        ...
        self.fig.suptitle(
            "Phase Sapping Dynamics: The Mechanism of Dimensional Emergence",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def update_plot(self):
        ...
        for d in range(max_vis_dim):
            ...
            if phase_mag > 0.01:
                ...
                self.ax3d.plot(
                    [x, x], [y, y], [z, z + bar_height], "b-", linewidth=6, alpha=0.7
                )
        ...
        if len(self.dynamics.history) > 10:
            ...
            if len(trail_coords) > 1:
                ...
                self.ax3d.plot(
                    ...
                    linewidth=3,
                    alpha=0.8,
                    label="Dimensional Evolution",
                )

        # Current state info
        info_text = (
            f"Time: {self.dynamics.time:.1f}\n"
