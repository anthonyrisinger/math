...
def parse_args():
    ...
    p.add_argument(
        "--nk", type=int, default=81, help="number of k slices (default: 81)"
    )
    p.add_argument("--nt", type=int, default=480, help="θ resolution (default: 480)")
    p.add_argument(
        "--lim", type=float, default=1.8, help="XY half-extent (default: 1.8)"
    )
    p.add_argument(
        "--output", "-o", default="morphic_views.png", help="output PNG file"
    )
    p.add_argument(
        "--transparent", action="store_true", help="transparent figure background"
    )
    return p.parse_args()
...
def main():
    ...
    for elev, azim, title, cell in views:
        ...
        ax.plot(
            circ[:, 0], circ[:, 1], zs=k_circle, zdir="z", ls="--", color="gray", lw=1.2
        )

        # surface (θ×k)
        ax.plot_surface(
            X, Y, Z, facecolors=FC, rstride=1, cstride=1, antialiased=True, shade=False
        )
    ...
    print(
        f"[✓] {args.output} written — mode={mode}, k∈[{k_min},{k_max}], θ={N_t}, k-slices={N_k}"
    )
