...
from core_measures import DimensionalMeasures, print_critical_info, setup_3d_axis
...
class DimensionalLandscape:
    ...
    def create_figure(self):
        ...
        self.fig.suptitle(
            "Dimensional Landscape: Where Geometry Meets Reality",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def _create_controls(self):
        ...
        self.dim_slider = Slider(
            ax_dim, "Dimension", 0.1, self.d_max, valinit=self.current_d, valfmt="%.2f"
        )
    ...
    def update_plot(self):
        ...
        if self.show_critical:
            ...
            for d_crit, label, color, marker in critical_dims:
                if d_crit <= self.d_max:
                    ...
                    # Label
                    self.ax3d.text(
                        d_crit, 0, c * 0.06, f" {label}", fontsize=9, color=color
                    )
        ...
        if self.show_volume:
            ...
            self.ax3d.plot(
                [self.current_d] * len(trajectory_phi),
                traj_y,
                traj_z,
                "b-",
                linewidth=3,
                alpha=0.8,
                label=f"V={v_current:.2f}",
            )
