...
def set_canonical_ortho(
    ax, elev: float = CAMERA_ELEV_DEG, azim: float = CAMERA_AZIM_DEG
):
...
def format_badge(
    name: str, value: float, policy: ResidualPolicy = ResidualPolicy()
) -> tuple[str, str]:
    """
...
class HealthState:
    backend: str
    Nk: int | None = None
    Nk_level: str | None = None  # 'ok'|'warn'|'bad'
    gauge_ok: bool | None = None
    fps: float | None = None
    hints: str | None = None  # free-form, e.g., 'QT_QPA_PLATFORM=xcb on Wayland'


def nk_level(Nk: int | None) -> str | None:
    if Nk is None:
        return None
    if Nk >= 121:
        return "ok"
...
class StyleContract:
    ...
    convergence_key: str | None = "Nk"  # e.g., 'Nk' or 'Nt' or None
...
def render_badge(
    ...
    name: str,
    value: float,
    policy: ResidualPolicy = ResidualPolicy(),
