...
```bash
# Store UUID for session
UUID=$(uuidgen)

# First message only - use --session-id
claude --print --permission-mode bypassPermissions --append-system-prompt '.github/copilot-instructions.md ROLE is LEAD ARCHITECT.' --session-id "$UUID" <<'EOF_MESSAGE'
Session UUID: $UUID
[Your message content]
EOF_MESSAGE

# All subsequent messages - use --resume
claude --print --permission-mode bypassPermissions --append-system-prompt '.github/copilot-instructions.md ROLE is LEAD ARCHITECT.' --resume "$UUID" <<'EOF_MESSAGE'
Session UUID: $UUID
[Your message content]
EOF_MESSAGE
```
...
**CRITICAL**: Store UUID in shell variable once at session start. Use fixed `EOF_MESSAGE` delimiters for security. Include UUID inside message content for session context.
