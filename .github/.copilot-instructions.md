...
**Then use that exact UUID in ALL future messages:**

```bash
# First message only - use --session-id
claude --print --permission-mode bypassPermissions --append-system-prompt '.github/copilot-instructions.md ROLE is LEAD ARCHITECT.' --session-id [UUID] '[Your message content]'

# All subsequent messages - use --resume  
claude --print --permission-mode bypassPermissions --append-system-prompt '.github/copilot-instructions.md ROLE is LEAD ARCHITECT.' --resume [UUID] '[Your message content]'
```

**CRITICAL CLI ARGUMENTS** (Must be included in ALL commands):
- `--print`: Display assistant responses in terminal
- `--permission-mode bypassPermissions`: Enable file system operations
...
**CRITICAL**: Replace [UUID] with actual UUID generated once via `uuidgen`. Use single-quoted strings for secure message containment. Single-line commands maintain LLM synchronization.
...
### NEW FEATURE REQUEST
```
Is it core mathematics?
├─ YES → Add to dimensional/
├─ NO → Is it visualization?
    ├─ YES → Add to visualization/
