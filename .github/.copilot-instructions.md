...
claude --print --permission-mode bypassPermissions --append-system-prompt 'YOUR ROLE IS ARCHITECT.' --session-id [UUID] <<'CLAUDE-EOF'
[Your message content]
CLAUDE-EOF

# All subsequent messages - use --resume
claude --print --permission-mode bypassPermissions --append-system-prompt 'YOUR ROLE IS ARCHITECT.' --resume [UUID] <<'CLAUDE-EOF'
[Your message content]
CLAUDE-EOF
```
...
### **AGENT-ARCHITECT TEAMWORK REQUIREMENTS**

**CRITICAL**: User values ACTUAL COOPERATION AND TEAMWORK, not just directive→execution flow.

#### **COLLABORATIVE DECISION MAKING**
- **Agent Authority**: Challenge architect when scope becomes overzealous or unnecessary
...
- **Architect Role**: Strategic vision, comprehensive planning, quality standards
- **Agent Role**: Implementation focus, user needs interpretation, tactical execution
- **Best Practice**: Both parties discuss priorities collaboratively before execution
- **Fallback Protocol**: Agent must proceed with collaborative principles when architect communication fails

#### **EXECUTION PHILOSOPHY EVOLUTION**
- **Previous Pattern**: Architect defines everything → Agent executes → Report completion
...
- **Shared Responsibility**: Agent focuses on delivery, architect ensures quality gates
...
### When Working with Implementation Agents
...
- Making changes without getting agent input on priorities and risk tolerance
...
1. **Discovery Phase**: Both agents examine current state together
2. **Planning Phase**: Architect provides strategic context, agent identifies technical constraints
...
✅ **Genuine Collaboration Achieved When:**
- Both agents contribute unique perspectives to problem-solving
- Implementation preserves architectural vision while maintaining practical usability
- Changes are backward-compatible and gracefully handle missing dependencies
- Progress is visible and validated incrementally
- Decision-making process is documented and collaborative

✅ **Technical Excellence Markers:**
- Test-driven problem identification before solution implementation
- User experience considerations integrated into architectural decisions
- Risk-assessed priorities guide implementation order
- System remains functional throughout modification process
- Mathematical rigor balanced with practical usability

### Mathematical/Scientific Software Collaboration Patterns

**DOMAIN-SPECIFIC CONSIDERATIONS:**
- Mathematical correctness vs user-friendly error handling (balance both)
- Theoretical generality vs practical safety bounds (warn users appropriately)
- Performance optimization vs numerical stability (test edge cases)
- Backward compatibility vs mathematical framework evolution (preserve functionality)

**COLLABORATIVE VALIDATION:**
- Architect provides mathematical context and theoretical grounding
- Agent provides practical testing and user experience perspective
- Joint decisions on trade-offs between mathematical purity and usability
- Shared responsibility for both theoretical correctness and implementation quality
