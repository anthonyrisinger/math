...
from core.phase import sap_rate, phase_evolution_step
...
class TestSapRateProperties:
    ...
    def test_sap_rate_directionality(self, source, target):
        ...
        if source < target:
            ...
            assert rate_forward >= 0, f"Negative forward sap rate: {source}→{target} = {rate_forward}"
            # Backward sapping should be zero
            assert rate_backward == 0, f"Non-zero backward sap rate: {target}→{source} = {rate_backward}"
        elif source > target:
            # Forward sapping should be zero
            assert rate_forward == 0, f"Non-zero downward sap rate: {source}→{target} = {rate_forward}"
        else:
            # Same dimension: both should be zero
            assert rate_forward == 0 and rate_backward == 0, f"Non-zero self-sap: {source}→{source}"
    
    @given(st.floats(min_value=0.0, max_value=8.0, allow_nan=False, allow_infinity=False),
    ...
    def test_sap_rate_equilibrium_saturation(self, source, density):
        ...
        if rate_low > NUMERICAL_EPSILON:
            assert rate_high <= rate_low + NUMERICAL_EPSILON, f"Sap rate should decrease with higher density: {rate_low} → {rate_high}"
    ...
    def test_sap_rate_distance_scaling(self, distance):
        ...
        if rate1 > NUMERICAL_EPSILON and rate2 > NUMERICAL_EPSILON:
            ...
            assert rate2 <= rate1 * 2, f"Sap rate scaling issue: d={distance} gives {rate1}, d={2*distance} gives {rate2}"


class TestPhaseEvolutionConservation:
    """Test conservation laws in phase evolution"""
    
    @given(arrays(dtype=np.float64, shape=st.integers(3, 10), 
    ...
    def test_energy_conservation(self, initial_energies):
        ...
        assert relative_change < 1e-10, f"Energy not conserved: {initial_total_energy} → {final_total_energy} (change: {relative_change})"
    ...
    def test_phase_evolution_stability(self, n_dims, dt):
        ...
        for step in range(10):
            ...
            assert np.all(np.isfinite(current_state)), f"Non-finite values at step {step}"
            ...
            assert max_energy < 100, f"Explosive growth at step {step}: max_energy = {max_energy}"
        ...
        for i, total in enumerate(total_energies):
            ...
            assert relative_change < 1e-8, f"Energy drift at step {i}: {relative_change}"
    ...
    def test_phase_evolution_irreversibility(self, n_dims):
        ...
        initial_concentration = initial_energies[0]
        final_concentration = final_energies[0]
        
        # Some energy should have moved to higher dimensions
        higher_dim_energy = np.sum(final_energies[1:])
        assert higher_dim_energy > 1e-6, f"No energy transfer to higher dimensions: {higher_dim_energy}"
        ...
        assert relative_change < 1e-8, f"Energy not conserved in irreversibility test: {relative_change}"


class TestPhaseCoherenceProperties:
    """Test phase coherence and interference properties"""
    
    @given(st.floats(min_value=0.0, max_value=2*np.pi, allow_nan=False, allow_infinity=False),
    ...
    def test_phase_invariance_under_global_rotation(self, global_phase, amplitude):
        ...
        assert energy_difference < 1e-12, f"Global phase rotation not preserved: max diff = {energy_difference}"
        ...
        phase_diff_change = np.abs(np.angle(np.exp(1j * (phase_diff_evolved - phase_diff_initial))))
        max_phase_drift = np.max(phase_diff_change)
        assert max_phase_drift < 0.1, f"Phase relationships not preserved: max drift = {max_phase_drift}"
