...
try:
    from .gamma import gamma_safe, factorial_extension
    ...
    import os
    from core.gamma import gamma_safe, factorial_extension
    from core.measures import ball_volume, sphere_surface, complexity_measure
    from core.performance import PerformanceProfiler, PropertyValidator
...
class ScalabilityValidator:
    ...
    def test_concurrent_execution(self) -> ProductionReadinessScore:
        ...
        if thread_speedup > 1.5:
            ...
            recommendations.append("Limited threading benefit - check for GIL limitations")
    ...
    def _verify_results_consistency(self, seq_results, thread_results, proc_results=None) -> bool:
    ...
    def _calculate_memory_efficiency(self, sizes: List[int], memory_usage: List[float]) -> float:
            ...
            try:
                ...
                if expected == "finite_positive":
                    if not (np.isfinite(result) and result > 0):
                        score -= 0.1
                        issues.append(f"Expected finite positive for {description}, got {result}")

                elif expected == "handle_pole" or expected == "handle_infinite":
                    if not (np.isinf(result) or np.isnan(result) or abs(result) > 1e10):
                        score -= 0.05
                        recommendations.append(f"Consider special handling for {description}")

                elif expected == "handle_nan":
                    if not np.isnan(result):
                        score -= 0.05
                        recommendations.append(f"NaN input should produce NaN output for {description}")

                elif expected == "handle_negative":
                    if not (np.isnan(result) or result == 0):
                        score -= 0.1
                        issues.append(f"Negative dimension should be handled gracefully for {description}")
                ...
                if expected in ["handle_pole", "handle_negative", "handle_complex"]:
                    ...
                    score -= 0.05
                    recommendations.append(f"Consider graceful handling for {description}: {str(e)}")
...
def comprehensive_production_assessment():
    ...
    # Overall assessment
    overall_score = sum(assessment.score for assessment in assessments.values()) / len(assessments)
