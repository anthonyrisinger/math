...
class LayoutManager:
    ...
    def create_dashboard_figure(self):
        ...
        if self.layout == 'grid':
            ...
            self.main_fig.suptitle('Dimensional Emergence Framework Dashboard',
...
class TopologyViewController:
    ...
    def __init__(self):
        ...
        self.scene_categories = {
            ...
            'QWZ Models': ['qwz_curvature', 'bz_torus_curvature_cloud', 'bz_torus_bulged', 'qwz_cylinder_bands'],
...
class DimensionalDashboard:
    ...
    def _add_interactive_controls(self):
        ...
        self.dimension_slider = Slider(
            ax_dim, 'Dimension', 0.1, 12.0,
            valinit=self.state.dimension, valfmt='%.1f'
        )
        ...
        self.time_slider = Slider(
            ax_time, 'Time', 0.0, 10.0,
            valinit=self.state.time, valfmt='%.1f'
        )
    ...
    def _update_landscape_view(self):
        ...
        surfaces = np.array([self.landscape.measures.sphere_surface(d) for d in d_range])
    ...
    def _render_controls_info(self):
        ...
        # Topology scene info
        ax.text(0.1, 0.9, 'Topology Scene:', fontweight='bold', transform=ax.transAxes, fontsize=10)
        scene_display = self.topo_controller.current_scene.replace('_', ' ').title()
        ax.text(0.1, 0.8, f'Current: {scene_display}', transform=ax.transAxes, fontsize=9)
        
        if TOPO_VIZ_AVAILABLE:
            ax.text(0.1, 0.7, f'Available: {len(AVAILABLE_SCENES)} scenes', transform=ax.transAxes, fontsize=8)
            ax.text(0.1, 0.6, 'Categories:', fontweight='bold', transform=ax.transAxes, fontsize=9)
            categories = list(self.topo_controller.get_available_scenes().keys())[:3]
            for i, cat in enumerate(categories):
                ax.text(0.1, 0.55 - i*0.08, f'• {cat}', transform=ax.transAxes, fontsize=8)
        
        # Current parameters
        ax.text(0.1, 0.35, 'Parameters:', fontweight='bold', transform=ax.transAxes, fontsize=9)
        ax.text(0.1, 0.28, f'd = {self.state.dimension:.1f}', transform=ax.transAxes, fontsize=8)
        if hasattr(self.topo_controller, 'scene_params') and self.topo_controller.scene_params:
            params_str = ', '.join([f'{k}={v:.1f}' if isinstance(v, (int, float)) else f'{k}={v}' 
        ...
        ax.text(0.1, 0.15, 'Controls:', fontweight='bold', transform=ax.transAxes, fontsize=9)
        ax.text(0.1, 0.08, '• n/p: Switch scenes', transform=ax.transAxes, fontsize=8)
        ax.text(0.1, 0.01, '• Sliders: Adjust params', transform=ax.transAxes, fontsize=8)
        ...
        if TOPO_VIZ_AVAILABLE and len(AVAILABLE_SCENES) > 1:
            ...
            def on_key_press(event):
                if event.key == 'n':  # Next scene
                    self.current_scene_index = (self.current_scene_index + 1) % len(self.scene_list)
