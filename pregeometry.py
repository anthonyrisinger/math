...
from core_measures import setup_3d_axis, PHI, PI, PSI
        ...
        for n in self.n_range:
            if n > -0.5 and n < 0.5:
                ...
                try:
                    # Complex extension
                    phase = PI * n
                    magnitude = abs(1 / gamma(n/2 + 1)) if n < 0 else PI**(n/2) / abs(gamma(n/2 + 1))
                    v = magnitude * np.exp(1j * phase)
                    
                    magnitude_s = abs(2 / gamma(n/2)) if n < 0 else 2 * PI**(n/2) / abs(gamma(n/2))
...
class PreGeometryVisualizer:
    ...
    def create_figure(self):
        ...
        self.fig.suptitle('Pre-Geometry: The n=-1 Primordial State Before Dimension',
    ...
    def update_plots(self):
        ...
        self.ax3d.plot_surface(X, Y, Z, facecolors=cm.hsv(colors_normalized),
                              alpha=0.8, linewidth=0, antialiased=True)
        
        # Mark critical dimensions
        for n_crit, label, color in [(-1, 'n=-1', 'white'),
                                     (0, 'void', 'black'),
                                     (1, 'd=1', 'blue')]:
            if -1 <= n_crit <= 1:
                self.ax3d.plot([n_crit, n_crit], [0, 0], [-1, 1],
        ...
        self.ax3d.scatter([-1], [0], [osc_height], c='gold', s=200, marker='*',
        ...
        self.ax2d.plot(self.pregeom.n_range, np.real(self.pregeom.psi), 
                      'b-', alpha=0.7, label='Re(Ψ)')
        self.ax2d.plot(self.pregeom.n_range, np.imag(self.pregeom.psi),
                      'r-', alpha=0.7, label='Im(Ψ)')
        self.ax2d.plot(self.pregeom.n_range, np.abs(self.pregeom.psi),
                      'k-', linewidth=2, label='|Ψ|')
        
        # Fill probability density
        prob = self.pregeom.get_probability_density()
        self.ax2d.fill_between(self.pregeom.n_range, 0, prob,
                              alpha=0.3, color='purple', label='|Ψ|²')
        ...
        self.ax2d.axvline(PHI-1, color='green', linestyle='--', alpha=0.5, label='φ-1')
        ...
        # Add info text
        info_text = (f"Time: {self.pregeom.time:.2f}\n"
                    f"⟨n⟩ = {np.sum(self.pregeom.n_range * prob):.3f}\n"
                    f"σ_n = {np.sqrt(np.sum((self.pregeom.n_range**2) * prob)):.3f}")
        
        self.ax2d.text(0.02, 0.98, info_text, transform=self.ax2d.transAxes,
