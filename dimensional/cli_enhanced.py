...
def measure_command(
    dimension: Annotated[
        float, typer.Argument(help="Dimensional parameter d ≥ 0")
    ] = 4.0,
    ...
    precision: Annotated[
        int, typer.Option("--precision", "-p", help="Decimal precision")
    ] = 12,
    format: Annotated[
        Literal["table", "json"], typer.Option("--format", "-f", help="Output format")
    ] = "table",
) -> None:
...
def explore_command(
    start: Annotated[
        float, typer.Option("--start", "-s", help="Start dimension")
    ] = 0.0,
    end: Annotated[float, typer.Option("--end", "-e", help="End dimension")] = 10.0,
    points: Annotated[
        int, typer.Option("--points", "-n", help="Number of points")
    ] = 100,
    mode: Annotated[
    ...
    try:
        ...
        with Progress(
            ...
            explore_task = progress.add_task(
                f"[cyan]Exploring dimensions {start:.1f} → {end:.1f}", total=points
            )
...
def gamma_command(
    ...
    show_properties: Annotated[
        bool, typer.Option("--props", help="Show gamma properties")
    ] = False,
) -> None:
    ...
    try:
        ...
        if "pole" in str(e).lower():
            console.print(
                "[yellow]💡 Tip: Gamma function is undefined at negative integers[/yellow]"
            )
        raise typer.Exit(1)
...
def phase_command(
    ...
    time_steps: Annotated[
        int, typer.Option("--steps", "-s", help="Evolution steps")
    ] = 100,
    show_evolution: Annotated[
        bool, typer.Option("--evolution", help="Show time evolution")
    ] = False,
) -> None:
...
def critical_command() -> None:
    ...
    tree = Tree(
        "🎯 [bold cyan]Critical Dimensions[/bold cyan]", guide_style="bright_blue"
    )
...
[cyan]Critical points:[/cyan] {len(results['critical_points'])}"""
    ...
    console.print(
        Panel(panel_content, title="🔍 Exploration Summary", border_style="blue")
    )
...
def _display_phase_state(state: PhaseState, title: str) -> None:
    ...
    energy_status = (
        "🔋 High"
        if state.energy > 0.7
        else "🪫 Low" if state.energy < 0.3 else "⚡ Medium"
    )
...
def _show_phase_evolution(
    engine: PhaseDynamicsEngine, initial_state: PhaseState, steps: int
) -> None:
...
def main() -> None:
    ...
    try:
        # Add CLI metadata
        console.print("[dim]Type-Safe Dimensional Mathematics CLI v1.0[/dim]")
        console.print(
            "[dim]Rich terminal interface with mathematical validation[/dim]\n"
        )
