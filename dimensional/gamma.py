...
v = lambda d: π**(d/2) / gamma_safe(d/2 + 1)                    # Volume of d-ball
s = lambda d: 2*π**(d/2) / gamma_safe(d/2)                      # Surface of (d-1)-sphere  
c = lambda d: v(d) * s(d)                                       # Complexity V×S
r = lambda d: s(d) / v(d) if v(d) > 0 else float('inf')        # Ratio S/V
ρ = lambda d: gamma_safe(d/2 + 1) / π**(d/2)                   # Density 1/V
...
ψ1 = lambda x: polygamma_safe(1, x)      # Trigamma
ψ2 = lambda x: polygamma_safe(2, x)      # Tetragamma
ψn = lambda n, x: polygamma_safe(n, x)   # n-th derivative

# Complex gamma
γ = lambda z: gamma_safe(z)              # Works with complex z
log_γ = lambda z: gammaln_safe(z) if np.isreal(z) else np.log(gamma_safe(z))
abs_γ = lambda z: np.abs(gamma_safe(z))

# Special formulas
reflect = lambda z: π / (np.sin(π*z) * gamma_safe(1-z))  # Reflection formula
duplicate = lambda z: np.sqrt(π) * 2**(1-2*z) * gamma_safe(2*z) / gamma_safe(z+0.5)  # Duplication
...
class GammaLab:
    ...
    def update(self):
        ...
        # Title
        self.fig.text(0.5, 0.96, f'GAMMA LAB | d = {self.d:.3f} | Mode: {self.modes[self.mode].upper()}',
    ...
    def plot_measures(self):
        ...
        ax1.set_title(f'Volume V_d = π^(d/2)/Γ(d/2+1) | V_{self.d:.1f} = {v(self.d):.4f}', color='cyan')
        ...
        ax2.set_title(f'Surface S_d = 2π^(d/2)/Γ(d/2) | S_{self.d:.1f} = {s(self.d):.4f}', color='magenta')
        ...
        ax3.text(peak_d, c_vals[peak_idx], f'  Peak: d={peak_d:.3f}', color='red', fontsize=10)
        
        ax3.set_title(f'Complexity C_d = V_d × S_d | C_{self.d:.1f} = {c(self.d):.4f}', color='lime')
    ...
    def plot_gamma(self):
        ...
        ax1.set_title(f'Γ(x) | Γ({self.d/2:.2f}) = {gamma_safe(self.d/2):.4f}', color='cyan')
        ax1.set_ylim(-10, 10)
        ax1.grid(True, alpha=0.2)
        ...
        ax2.set_title(f'log Γ(x)', color='magenta')
        ax2.grid(True, alpha=0.2)
        ...
        ax3.set_title(f'ψ(x) = d/dx log Γ(x)', color='lime')
        ax3.grid(True, alpha=0.2)
    ...
    def plot_poles(self):
        ...
        # Plot surface
        surf = ax.plot_surface(X, Y, Gamma_vals, cmap='plasma', alpha=0.8)
...
class LiveGamma:
    ...
    def __init__(self, expr_file="gamma_expr.py"):
        ...
        if not os.path.exists(self.expr_file):
            ...
            print(f"✨ Created {self.expr_file} - edit it and save to see live updates!")
...
__all__ = [
    # Core functions
    'gamma_safe', 'gammaln_safe', 'digamma_safe', 'polygamma_safe',
    # Quick tools
    'v', 's', 'c', 'r', 'ρ', 'find_peak', 'v_peak', 's_peak', 'c_peak',
    'ψ0', 'ψ1', 'ψ2', 'ψn', 'γ', 'log_γ', 'abs_γ', 'reflect', 'duplicate',
    # Visualization
    'qplot', 'qsurf', 'explore', 'peaks', 'instant', 
    # Interactive
    'GammaLab', 'LiveGamma', 'lab', 'live', 'demo',
    # Constants
    'PI', 'PHI', 'PSI', 'E', 'SQRT_PI', 'π', 'φ', 'ψ'
]
