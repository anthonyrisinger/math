...
def test_constants():
    ...
    from core import PHI, PI, PSI, VARPI, CRITICAL_DIMENSIONS
    ...
    tests = [
        ...
        (len(CRITICAL_DIMENSIONS) > 5, f"Has {len(CRITICAL_DIMENSIONS)} critical dimensions")
    ]
...
def test_dimensional_measures():
    ...
    from core import ball_volume, sphere_surface, complexity_measure
...
def test_phase_dynamics():
    ...
    from core import sap_rate, PhaseDynamicsEngine
    ...
    try:
        ...
        relative_change = energy_change / initial_energy if initial_energy > 0 else energy_change
        if relative_change < 1.0:  # Allow significant energy change for emergence dynamics
            print(f"✅ Energy change within expected range (relative change: {relative_change:.1%})")
        else:
            print(f"⚠️ Large energy change: {relative_change:.1%} (may indicate numerical instability)")
        ...
        if np.isfinite(final_state['total_energy']) and final_state['total_energy'] >= 0:
...
def test_morphic_mathematics():
    ...
    from core.morphic import golden_ratio_properties, discriminant, k_perfect_circle
...
def test_performance():
    ...
    from core import ball_volume, PhaseDynamicsEngine
    ...
    print(f"📊 1000 ball volume calculations: {ball_time:.4f}s ({1000/ball_time:.0f} calc/s)")
    ...
    print(f"📊 100 phase dynamics steps: {phase_time:.4f}s ({100/phase_time:.0f} steps/s)")
