...
from core.morphic import morphic_polynomial_roots, discriminant
from core.constants import PHI, PSI, NUMERICAL_EPSILON
...
class TestGoldenRatioProperties:
    ...
    def test_golden_ratio_definition(self):
        ...
        assert abs(PHI - expected) < NUMERICAL_EPSILON, f"φ definition failed: {PHI} ≠ {expected}"
    
    def test_golden_conjugate_definition(self):
        """Test ψ = 1/φ"""
        expected = 1 / PHI
        assert abs(PSI - expected) < NUMERICAL_EPSILON, f"ψ definition failed: {PSI} ≠ {expected}"
    ...
    def test_golden_ratio_quadratic(self):
        ...
        assert error < NUMERICAL_EPSILON, f"φ² = φ + 1 failed: {phi_squared} ≠ {phi_plus_one}"
    ...
    def test_golden_conjugate_quadratic(self):
        ...
        assert error < NUMERICAL_EPSILON, f"ψ² = 1 - ψ failed: {psi_squared} ≠ {one_minus_psi}"
    ...
    def test_golden_ratio_sum(self):
        ...
        assert error < NUMERICAL_EPSILON, f"φ + ψ = √5 failed: {sum_phi_psi} ≠ {expected}"
...
class TestMorphicPolynomialProperties:
    ...
    def test_shifted_polynomial_root_verification(self, k):
        ...
        for root in roots:
            ...
            assert abs(poly_value) < 1e-10, f"Root {root} doesn't satisfy shifted polynomial for k={k}: f({root}) = {poly_value}"
    ...
    def test_simple_polynomial_root_verification(self, k):
        ...
        for root in roots:
            ...
            assert abs(poly_value) < 1e-10, f"Root {root} doesn't satisfy simple polynomial for k={k}: f({root}) = {poly_value}"
    ...
    def test_polynomial_discriminant_sign_consistency(self, k):
        ...
        if disc_shifted > NUMERICAL_EPSILON:
            assert len(roots_shifted) == 3, f"Shifted: Δ > 0 but {len(roots_shifted)} real roots for k={k}"
        elif disc_shifted < -NUMERICAL_EPSILON:
            assert len(roots_shifted) == 1, f"Shifted: Δ < 0 but {len(roots_shifted)} real roots for k={k}"
        
        if disc_simple > NUMERICAL_EPSILON:
            assert len(roots_simple) == 3, f"Simple: Δ > 0 but {len(roots_simple)} real roots for k={k}"
        elif disc_simple < -NUMERICAL_EPSILON:
            assert len(roots_simple) == 1, f"Simple: Δ < 0 but {len(roots_simple)} real roots for k={k}"
...
class TestMorphicStabilityProperties:
    ...
    def test_root_ordering_consistency(self, k):
        ...
        for i in range(len(roots_shifted) - 1):
            assert roots_shifted[i] >= roots_shifted[i + 1], f"Shifted roots not in descending order at k={k}"
        
        for i in range(len(roots_simple) - 1):
            assert roots_simple[i] >= roots_simple[i + 1], f"Simple roots not in descending order at k={k}"
    ...
    def test_morphic_continuity(self, k):
        ...
        if len(roots_k) == len(roots_k_plus_eps):
            for i in range(len(roots_k)):
                ...
                assert root_change < 0.1, f"Large root change {root_change} for small k change at k={k}"


class TestMorphicGeometricInvariants:
    """Test invariant properties under morphic transformations"""
    
    @given(st.floats(min_value=0.5, max_value=5.0, allow_nan=False, allow_infinity=False))
