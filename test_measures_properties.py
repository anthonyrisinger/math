...
from core.measures import ball_volume, sphere_surface, complexity_measure, phase_capacity
...
class TestDimensionalMeasureBasicProperties:
    ...
    def test_volume_positivity(self, d):
        ...
        vol = ball_volume(d)
        assert vol > 0 or np.isclose(vol, 0, atol=NUMERICAL_EPSILON), f"Negative volume at d={d}: {vol}"
    ...
    def test_surface_positivity(self, d):
        ...
        assert surf > 0 or np.isclose(surf, 0, atol=NUMERICAL_EPSILON), f"Negative surface at d={d}: {surf}"
        assert np.isfinite(surf), f"Non-finite surface at d={d}: {surf}"
    
    @given(st.floats(min_value=0.1, max_value=15.0, allow_nan=False, allow_infinity=False))
    ...
    def test_complexity_positivity(self, d):
        ...
        if comp == 0:
            ...
            if vol > NUMERICAL_EPSILON and surf > NUMERICAL_EPSILON:
                assert False, f"Unexpected zero complexity at d={d} with V={vol}, S={surf}"
...
class TestKnownValueVerification:
    ...
    def test_volume_integer_dimensions(self):
        ...
        # V_3 = 4π/3
        expected_v3 = 4 * PI / 3
        assert abs(ball_volume(3) - expected_v3) < NUMERICAL_EPSILON, f"V_3 ≠ 4π/3: {ball_volume(3)} ≠ {expected_v3}"
...
class TestRecurrenceRelations:
    ...
    def test_volume_recurrence(self, d):
        ...
        assert relative_error < tolerance, f"Volume recurrence failed at d={d}: V_{d+2} = {vol_d_plus_2} ≠ {expected}"
    
    @given(st.floats(min_value=1.5, max_value=15.0, allow_nan=False, allow_infinity=False))
    @settings(max_examples=300)
    def test_surface_volume_relation(self, d):
        ...
        assert relative_error < tolerance, f"Surface-volume relation failed at d={d}: S_d = {surf_d} ≠ d×V_d = {expected}"


class TestScalingProperties:
    """Test scaling behavior of measures"""
    
    @given(st.floats(min_value=0.5, max_value=10.0, allow_nan=False, allow_infinity=False),
    ...
    def test_volume_scaling(self, d, scale):
        ...
        scaled_volume_expected = unit_volume * (scale ** d)
        ...
        if scale > 1:
            assert ratio >= 1, f"Volume should increase with scale > 1"
        elif scale < 1:
            assert ratio <= 1, f"Volume should decrease with scale < 1"
    ...
    def test_surface_scaling(self, d, scale):
        ...
        if d > 1:  # For d > 1, surface scaling behaves normally
            if scale > 1:
                assert ratio >= 1, f"Surface should increase with scale > 1 when d > 1"
            elif scale < 1:
                assert ratio <= 1, f"Surface should decrease with scale < 1 when d > 1"
...
class TestAsymptoticBehavior:
    ...
    def test_volume_large_d_monotonicity(self, d):
        ...
        assert ratio < 2.0, f"Volume increasing too rapidly at large d={d}: V_{d+1}/V_d = {ratio}"
...
class TestComplexityMeasureProperties:
    ...
    def test_complexity_factorization(self, d):
        ...
        assert relative_error < 1e-12, f"Complexity factorization failed at d={d}: C_d = {comp} ≠ V_d × S_d = {expected}"
    ...
    def test_complexity_peak_existence(self, d_center):
        ...
        assert relative_variation > 1e-10, f"Complexity appears constant around d={d_center}: variation = {relative_variation}"
