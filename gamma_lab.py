...
class GammaLab:
    ...
    def v(self, d):
        ...
        if abs(d) < 1e-10: return 1.0
        return π**(d/2) / gamma(d/2 + 1)
    
    def s(self, d):
        """Surface of unit (d-1)-sphere"""
        if abs(d) < 1e-10: return 2.0
    ...
    def plot_measures(self):
        ...
        ax1.set_title(f'Volume V_d = π^(d/2)/Γ(d/2+1) | V_{self.d:.1f} = {self.v(self.d):.4f}', color='cyan')
        ...
        ax2.set_title(f'Surface S_d = 2π^(d/2)/Γ(d/2) | S_{self.d:.1f} = {self.s(self.d):.4f}', color='magenta')
        ...
        ax3.set_title(f'Complexity C_d = V_d × S_d | C_{self.d:.1f} = {self.c(self.d):.4f}', color='lime')
