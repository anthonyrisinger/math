...
warnings.filterwarnings('error', category=RuntimeWarning)  # Mathematical warnings as errors
...
def golden_phase_engine():
    ...
    try:
        from dimensional import PhaseDynamicsEngine
        return PhaseDynamicsEngine(max_dimensions=6, use_adaptive=True)
    except ImportError:
        pytest.skip("PhaseDynamicsEngine not available")
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "visualization: marks visualization tests (may require display)")
