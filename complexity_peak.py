...
class ComplexityPeakExplorer:
    ...
    def __init__(self, focus_range=(3, 9), resolution=2000):
        ...
        self.surfaces = np.array(
            [self.measures.sphere_surface(d) for d in self.d_range]
        )
        ...
        print("COMPLEXITY PEAK DISCOVERED:")
        print(f"  Peak at d = {self.peak_dimension:.6f}")
        print(f"  Peak value = {self.peak_complexity:.6f}")
        print("  This is WHERE the universe maximizes information capacity!")
...
class ComplexityPeakVisualizer:
    ...
    def create_figure(self):
        ...
        self.fig.suptitle(
            "The Complexity Peak: Where Reality Maximizes Information Capacity",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def _create_controls(self):
        ...
        self.checkboxes = CheckButtons(
            ...
            [
                self.explorer.show_volume,
                self.explorer.show_surface,
                self.explorer.show_complexity,
                self.explorer.show_derivatives,
    ...
    def update_main_3d(self):
        ...
        for d_crit, label, color in [
            ...
            if self.explorer.focus_range[0] <= d_crit <= self.explorer.focus_range[1]:
                ...
                self.ax_main.text(d_crit, 0, 0.5, f" {label}", fontsize=10, color=color)

        # Current focus marker
        self.explorer.measures.ball_volume(self.explorer.focus_dim)
        self.ax_main.scatter(
            [self.explorer.focus_dim], [0], [0], c="red", s=200, marker="x", linewidth=4
        )

        # Info text
        focus_c = self.explorer.measures.complexity_measure(self.explorer.focus_dim)
        info_text = (
            f"Focus: d = {self.explorer.focus_dim:.3f}\n"
    ...
    def update_measures_plot(self):
        ...
        # Plot measures
        if self.explorer.show_volume:
            self.ax_measures.plot(
                self.explorer.d_range,
                self.explorer.volumes,
                "b-",
                linewidth=2,
                label="Volume",
                alpha=0.7,
        ...
        if self.explorer.show_complexity:
            self.ax_measures.plot(
                self.explorer.d_range,
                self.explorer.complexity,
                "r-",
                linewidth=3,
                label="Complexity (V×S)",
            )

        # Mark the peak
        self.ax_measures.scatter(
            [self.explorer.peak_dimension],
            [self.explorer.peak_complexity],
            c="gold",
            s=200,
            marker="*",
            zorder=5,
        ...
        # Focus line
        self.ax_measures.axvline(
            self.explorer.focus_dim, color="red", linewidth=2, alpha=0.7
        )
    ...
    def update_stability_plot(self):
        ...
        for i in range(len(self.explorer.d_range) - 1):
            ...
            self.ax_stability.plot(
                [d1, d2], [c1, c2], color=color, linewidth=3, alpha=0.7
            )
