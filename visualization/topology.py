...
def qwz_h(kx, ky, m):
    ...
    return np.array([[c1, sxy], [np.conjugate(sxy), -c1]], dtype=complex)
...
def chern_fhs_2d(h, Nk=81, params=None):
    ...
    def Ux(i, j):
        z = np.vdot(vecs[i, j, :], vecs[(i + 1) % Nk, j, :])
        return z / np.abs(z)

    def Uy(i, j):
        z = np.vdot(vecs[i, j, :], vecs[i, (j + 1) % Nk, :])
        return z / np.abs(z)
...
def scene_gamma_volume(n_max=15.0, steps=601, out=None, dpi=180, show=False):
    def vol_unit_ball(n):
        return mp.pi ** (n / 2) / mp.gamma(n / 2 + 1)
    ...
    ax.set_xlabel("dimension n (continuous)")
    ax.set_ylabel("Unit ball volume V_n(1)")
    ax.set_title("Gamma Dial: Unit Ball Volume vs dimension")
    ax.legend()
...
def scene_gamma_area(n_max=15.0, steps=601, out=None, dpi=180, show=False):
    ...
    def area_unit_sphere(n):
        return n * vol_unit_ball(n)
    ...
    ax.set_xlabel("dimension n (continuous)")
    ax.set_ylabel("Unit sphere area S_{n-1}(1)")
    ax.set_title("Gamma Dial: Unit Sphere Area vs dimension")
    ax.legend()
    return _save(fig, out, dpi, show)


def scene_simplex_orthant(n_max=6.0, steps=301, out=None, dpi=180, show=False):
    def vol_orthant_simplex(n):
        return 1.0 / mp.gamma(n + 1)
...
def scene_qwz_curvature(m=0.5, Nk=81, out=None, dpi=180, show=False):
    ...
    ax = fig.add_subplot(111)
    im = ax.imshow(
        curv.T, origin="lower", extent=[-np.pi, np.pi, -np.pi, np.pi], aspect="equal"
    )
    ax.set_xlabel("k_x")
    ax.set_ylabel("k_y")
    ax.set_title(
        f"QWZ Berry Curvature (m={m}) • Chern ≈ {ch:.6f} → {ci} (|Δ|={ce:.2e})"
    )
...
def scene_bz_torus_curvature_cloud(m=0.5, Nk=85, out=None, dpi=180, show=False):
    ...
    set_ortho(ax)
    ax.plot_wireframe(
        X[::6, ::3], Y[::6, ::3], Z[::6, ::3], rstride=1, cstride=1, linewidth=0.35
    )
...
def scene_bz_torus_bulged(
    m=0.5, Nk=85, R=0.7, r=0.25, alpha=0.10, out=None, dpi=180, show=False
):
    Nk = int(Nk)
    ks = np.linspace(-np.pi, np.pi, Nk, endpoint=False)
    U, V = np.meshgrid(ks + np.pi, ks + np.pi, indexing="ij")
    ...
    set_ortho(ax)
    ax.plot_wireframe(
        X[::6, ::3], Y[::6, ::3], Z[::6, ::3], rstride=1, cstride=1, linewidth=0.3
    )
    ax.plot_wireframe(
        Xb[::4, ::2], Yb[::4, ::2], Zb[::4, ::2], rstride=1, cstride=1, linewidth=0.9
    )
    ax.set_title(
        f"Curvature-Bulged Brillouin Torus (m={m}) • Chern ≈ {ch:.6f} → {ci} (|Δ|={ce:.2e})"
    )
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
...
def scene_qwz_cylinder_bands(Ny=60, Nk=181, m=1.0, out=None, dpi=180, show=False):
    ...
    for idx, kx in enumerate(kxs):
        ...
        w, v = np.linalg.eigh(H)
        evals[idx, :] = w
        left_mask = np.zeros((int(Ny),), dtype=float)
        left_mask[:3] = 1.0
        right_mask = np.zeros((int(Ny),), dtype=float)
        right_mask[-3:] = 1.0
    ...
    zc = np.linspace(np.min(Z), np.max(Z), 36)
    ...
    ax.set_title(
        f"QWZ Cylinder Bands (m={m}) • Bulk Chern ≈ {ch:.6f} → {ci} (|Δ|={ce:.2e})\nZero-energy isoplane; color encodes left↔right localization"
    )
    ax.set_xlabel("cos k_x")
    ax.set_ylabel("sin k_x")
...
def scene_wp_sphere_cuts(pairing="diag", N=7, out=None, dpi=180, show=False):
    ...
    def great_circle(a, b, num=240):
        ...
        if ln < 1e-9:
            return np.column_stack(
                [a[0] * np.ones(num), a[1] * np.ones(num), a[2] * np.ones(num)]
            )
        n = n / ln
    ...
    ax.set_title(
        f"Weierstrass Branch-Cut Relocation (pairing = {pairing}) — integers unchanged"
    )
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
...
def scene_torus_engine(
    R=0.6, r=0.3, Nu=160, Nv=60, p=2, q=1, out=None, dpi=180, show=False
):
    ...
    set_ortho(ax)
    ax.plot_wireframe(
        X[::6, ::3], Y[::6, ::3], Z[::6, ::3], rstride=1, cstride=1, linewidth=0.4
    )
    v_idx = X.shape[1] // 8
    u_idx = X.shape[0] // 6
...
def scene_torus_defects_ribbon(R=0.7, r=0.25, out=None, dpi=180, show=False):
    ...
    set_ortho(ax)
    ax.plot_wireframe(
        X[::8, ::3], Y[::8, ::3], Z[::8, ::3], rstride=1, cstride=1, linewidth=0.35
    )
...
def scene_pump_bloch(Nk=140, Nt=140, a=0.6, b=0.8, out=None, dpi=180, show=False):
    ...
    for k in ks:
        for t in ts:
            d = dvec(k, t, a=float(a), b=float(b))
            n = np.linalg.norm(d)
            if n == 0:
                continue
            D.append(d / n)
...
def scene_pump_loops(
    k0=0.3 * math.pi, t0=0.7 * math.pi, a=0.6, b=0.8, out=None, dpi=180, show=False
):
...
def scene_pump_chern(Nk=51, Nt=51, a=0.6, b=0.8, out=None, dpi=180, show=False):
    ...
    def chern_family(Nk=41, Nt=41):
        ...
        def U_k(i, j):
            z = np.vdot(vecs[i, j, :], vecs[(i + 1) % int(Nk), j, :])
            return z / np.abs(z)

        def U_t(i, j):
            z = np.vdot(vecs[i, j, :], vecs[i, (j + 1) % int(Nt), :])
            return z / np.abs(z)
        ...
        for i in range(int(Nk)):
            for j in range(int(Nt)):
                W = (
                    U_k(i, j)
                    * U_t((i + 1) % int(Nk), j)
                    * np.conjugate(U_k(i, (j + 1) % int(Nt)))
                    * np.conjugate(U_t(i, j))
                )
                F += np.angle(W)
    ...
    ax.axis("off")
    ax.text(
        0.02, 0.7, f"Pump Chern on (k,t): {C:.8f} → {Ci} (|Δ|={Ce:.2e})", fontsize=14
    )
...
def scene_hopf_link(out=None, dpi=180, show=False):
    ...
    def hopf_fiber_xyz(xi, phi, n=800):
        ...
        z1, z2 = hopf_zs(eta, xi, phi)
        x, y, z = stereographic_S3_to_R3(z1, z2)
        return x, y, z

    xi = np.pi / 2
    phi1, phi2 = 0.0, np.pi / 2
...
def scene_padic_tree(p=3, depth=5, out=None, dpi=180, show=False):
        ...
        for d in range(1, int(depth) + 1):
            ...
            for nid, _ in prev:
                for k in range(int(p)):
                    level.append((node_id, nid))
                    edges.append((nid, node_id))
                    node_id += 1
            levels.append(level)
        return edges, levels
    ...
    edges, levels = padic_tree_edges(p=int(p), depth=int(depth))
    coords = radial_layout(levels)
    fig = plt.figure(figsize=(7.2, 7.2))
    ax = fig.add_subplot(111)
    for u, v in edges:
        x0, y0 = coords[u]
    ...
    rings = [0.1 + 0.2 * d for d in range(len(levels))]
    th = np.linspace(0, 2 * np.pi, 180)
    for R in rings:
        ax.plot(R * np.cos(th), R * np.sin(th), linewidth=0.6)
    ax.set_aspect("equal", adjustable="box")
    ax.set_title(f"p-adic Toy: Clopen Shells (p={p}, depth={depth})")
    return _save(fig, out, dpi, show)
...
def _connect_keys(fig, sliders, save_cb, reset_cb, spin: SpinController | None):
    step = [getattr(s, "valstep", None) or 0.01 for s in sliders] + [
        0.01,
        0.01,
        0.01,
        0.01,
...
def scene_live_qwz(m=0.5, Nk=81, out=None, dpi=180, show=True):
    ...
    ci, ce = integer_snap(ch)
    ttl = ax.set_title(
        f"QWZ Curvature (m={m0:.3f}, Nk={Nk0}) • Chern≈{ch:.6f}→{ci} (|Δ|={ce:.2e})"
    )
    ax.set_xlabel("k_x")
    ax.set_ylabel("k_y")
    fig.colorbar(im, ax=ax, shrink=0.85)
    ...
    def _update(_):
        ...
        ci, ce = integer_snap(C)
        ttl.set_text(
            f"QWZ Curvature (m={mm:.3f}, Nk={NN}) • Chern≈{C:.6f}→{ci} (|Δ|={ce:.2e})"
        )
        fig.canvas.draw_idle()

    s_m.on_changed(_update)
    s_n.on_changed(_update)
    # Buttons
    ax_b = plt.axes([0.80, 0.02, 0.15, 0.07])
    btn = Button(ax_b, "Save PNG")
    ...
    if show:
        ...
        return _save(fig, out, dpi, show=False)
...
def scene_live_bz_torus_bulged(m=0.5, Nk=85, alpha=0.10, out=None, dpi=180, show=True):
    ...
    Nx, Ny, Nz = Nx / Nn, Ny / Nn, Nz / Nn
    curv_norm = curv / (np.max(np.abs(curv)) + 1e-12)
    Xb = X + a0 * curv_norm * Nx
    Yb = Y + a0 * curv_norm * Ny
    Zb = Z + a0 * curv_norm * Nz
    ax.plot_wireframe(
        Xb[::4, ::2], Yb[::4, ::2], Zb[::4, ::2], rstride=1, cstride=1, linewidth=0.9
    )
    ci, ce = integer_snap(ch)
    ttl = ax.set_title(
        f"Bulged BZ Torus (m={m0:.2f}, Nk={Nk0}, α={a0:.2f}) • Chern≈{ch:.6f}→{ci} (|Δ|={ce:.2e})"
    )
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
    ...
    def _redraw(mm, NN, aa):
        ...
        Nx, Ny, Nz = Nx / Nn, Ny / Nn, Nz / Nn
        curv_norm = curv / (np.max(np.abs(curv)) + 1e-12)
        Xb = X + aa * curv_norm * Nx
        Yb = Y + aa * curv_norm * Ny
        Zb = Z + aa * curv_norm * Nz
        ...
        ci, ce = integer_snap(ch)
        ttl.set_text(
            f"Bulged BZ Torus (m={mm:.2f}, Nk={NN}, α={aa:.2f}) • Chern≈{ch:.6f}→{ci} (|Δ|={ce:.2e})"
        )
        fig.canvas.draw_idle()
    ...
    for s in (s_m, s_n, s_a):
        s.on_changed(_update)
    # Save
    ax_b = plt.axes([0.65, 0.02, 0.29, 0.03])
    btn = Button(ax_b, "Save PNG")
...
def scene_live_pump(a=0.6, b=0.8, k0=0.9, t0=2.1, out=None, dpi=180, show=True):
    ...
    set_ortho(ax)
    ax.plot_wireframe(Xs[::4, ::4], Ys[::4, ::4], Zs[::4, ::4], linewidth=0.4)
    L1 = normed(d_bcast(k00, ts, a=a0, b=b0))
    L2 = normed(d_bcast(ks, t00, a=a0, b=b0))
    (l1,) = ax.plot3D(L1[0, :], L1[1, :], L1[2, :], linewidth=2.0)
    (l2,) = ax.plot3D(L2[0, :], L2[1, :], L2[2, :], linewidth=2.0)
    ttl = ax.set_title(
        f"Pump Loops (a={a0:.2f}, b={b0:.2f}, k0={k00:.2f}, t0={t00:.2f})"
    )
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
    ...
    for s in (s_a, s_b, s_k, s_t):
        s.on_changed(_update)
    ax_btn = plt.axes([0.88, 0.02, 0.09, 0.07])
...
def scene_live_wp(N=6, M=60, out=None, dpi=180, show=True):
    ...
    ax.set_xlabel("u")
    ax.set_ylabel("v")
    ax_r = plt.axes([0.78, 0.62, 0.18, 0.18])
    rb = RadioButtons(ax_r, labels=("diag", "adj"), active=0)
    ...
    rb.on_clicked(_pair)
    ax_s = plt.axes([0.12, 0.02, 0.7, 0.03])
    sN = Slider(ax_s, "sum N", 3, 12, valinit=N0, valstep=1)
    ...
    def _update(_):
        ...
        for i, u in enumerate(us):
            for j, v in enumerate(vs):
                ...
                mag[i, j] = 0 if np.isinf(w) else min(10.0, abs(w))
        im.set_data(mag.T)
        ax.set_title(f"Weierstrass ℘ Fundamental Domain (N={NN}, M={M0})")
        fig.canvas.draw_idle()

    sN.on_changed(_update)
    ax_b = plt.axes([0.83, 0.05, 0.13, 0.07])
    btn = Button(ax_b, "Save PNG")
...
def render(
    ...
    **kwargs,
    ...
    if scene_name not in SCENES:
        raise KeyError(
            f"Unknown scene '{scene_name}'. Available: {', '.join(list_scenes())}"
        )
...
def export_portfolio_zip(
    png_glob="fig_*.png", zip_path="Topological_Visual_Portfolio_pngs.zip"
):
    pngs = sorted(glob.glob(png_glob))
...
def parse_compact_cli(argv):
    ...
    for tok in argv:
        ...
        if low in ("figure", "fig", "interactive", "help"):
            mode = (
                "interactive"
                if low == "interactive"
                else ("figure" if low in ("figure", "fig") else low)
            )
            continue
    ...
    for tok in argv:
        ...
        if low in ("figure", "fig", "interactive", "help") or tok == scene:
            continue
        if _is_flag(tok) or (
            "=" in tok and low.split("=", 1)[0] in ("savefig", "dpi", "show", "out")
        ):
...
def _parse_kwargs(s: str):
    ...
    try:
        ...
        raise SystemExit(
            f"Could not parse --kwargs. Use a Python dict literal, e.g. \"{{'m': 1.0, 'Nk': 81}}\". Error: {e}"
        )
...
def main(argv=None):
    ...
    if argv and not any(a.startswith("--scene") or a == "--list" for a in argv):
        ...
        if parsed.get("mode") == "list" or parsed.get("mode") == "help":
            ...
            [print(" -", k) for k in list_scenes()]
            return 0
        out = parsed["out"] or (
            None if parsed["show"] else f"fig_{parsed['scene']}.png"
        )
        path = render(
            parsed["scene"],
            out=out,
        ...
        print(
            "Rendered (interactive):" if parsed["show"] else "Saved:",
            path or "(interactive)",
        )
        return 0
    ...
    ap.add_argument(
        ...
        type=str,
        required=False,
        help="Scene to render. Use --list to see options.",
    )
    ap.add_argument(
        "--kwargs",
        type=str,
        default="",
        help="Python dict of kwargs for the scene, e.g. \"{'m':1.0,'Nk':81}\"",
    )
    ap.add_argument(
        "--out",
        type=str,
        default="",
        help="Output image path (PNG). Defaults to ./fig_<scene>.png unless --show",
    )
    ap.add_argument("--dpi", type=int, default=180, help="Output DPI (default 180)")
    ap.add_argument(
        "--show", action="store_true", help="Also show the figure in a window"
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Build a portfolio PDF from existing PNGs matching fig_*.png",
    )
    ap.add_argument(
        "--portfolio-zip",
        type=str,
        default="",
        help="Zip existing PNGs matching fig_*.png",
    )
    args = ap.parse_args(argv)

    if args.list or not args.scene:
        print("Available scenes:")
        for k in list_scenes():
            print(" -", k)
        if not args.scene:
            return 0
    ...
    out = args.out or (
        None if args.show else os.path.join(os.getcwd(), f"fig_{args.scene}.png")
    )
    ...
    if args.portfolio_pdf:
        pdf_path = export_portfolio_pdf(
            png_glob="fig_*.png", pdf_path=args.portfolio_pdf
        )
        print("Portfolio PDF:", pdf_path)
    if args.portfolio_zip:
        zip_path = export_portfolio_zip(
            png_glob="fig_*.png", zip_path=args.portfolio_zip
        )
