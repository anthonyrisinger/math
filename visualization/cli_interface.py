...
from .modernized_dashboard import ModernDashboard, BackendType


@click.group()
@click.option('--backend', type=click.Choice(['kingdon', 'plotly', 'auto']), default='auto',
              help='Visualization backend to use')
@click.option('--dimension', type=float, default=4.0,
              help='Starting dimension value')
@click.option('--time', type=float, default=0.0, 
              help='Starting time value')
...
@viz.command()
@click.option('--layout', type=click.Choice(['mathematical_grid', 'unified', 'default']), 
              default='mathematical_grid', help='Dashboard layout')
@click.option('--export', type=click.Choice(['html', 'json', 'ganja', 'auto']),
...
@viz.command()
@click.option('--scene-type', type=click.Choice(['landscape', 'topology', 'morphic', 'cascade']),
              default='landscape', help='Scene type to render')
@click.option('--export', type=click.Choice(['html', 'json', 'ganja', 'auto']), 
...
def render(ctx, scene_type: str, export: str, output: str):
    ...
    # Render scene
    result = dashboard_obj.backend.render_scene(scene_data)
...
@viz.command()
@click.option('--control-type', type=click.Choice(['additive', 'multiplicative', 'boundary']),
              required=True, help='Control semantic type')
@click.option('--value', required=True, help='Control value (JSON format for complex values)')
@click.pass_context
def control(ctx, control_type: str, value: str):
    ...
    if success:
        click.echo(f"✅ Control applied successfully")
    else:
        click.echo(f"❌ Control application failed", err=True)
...
def info(ctx):
    ...
    if camera:
        ...
        click.echo(f"Golden ratio view: {info_data.get('orthographic_view', 'unknown')}")
...
@viz.command()
@click.option('--new-backend', type=click.Choice(['kingdon', 'plotly']), required=True,
...
@viz.command()
@click.option('--format', 'export_format', type=click.Choice(['html', 'json', 'ganja', 'auto']),
              default='auto', help='Export format')
@click.option('--output', '-o', type=click.Path(), required=True, help='Output file path')
@click.pass_context
